from input_dict import fcc_screw as config
...
def aniso_disl_constructor(main_path, config_style, dislocation_type, 
    ...
    if element_struct == 'hcp':
        atom_coor, atom_type, box_boundary, new_box_lattice_const, frame_new, repeat_para = \
                perfect_hcp_constructor(config, latt_const, pot_element, dislocation_type, unit_cell_size)
    if element_struct == 'fcc' or element_struct == 'bcc':
        atom_coor, atom_type, box_boundary = perfect_cryst_constructor(config, 
    ...
    if output_perfect == True:
        write_datafile(pot_element, dislocation_type, mass, directory, atom_coor, atom_type, box_boundary, suffix='_perfect_ref.dat')
    ...
    # write datafile
    write_datafile(pot_element, dislocation_type, mass, directory, atom_coor, atom_type, box_boundary)
    ...
    if simulation_type == 'energy_minimization':
        write_in_file(pot_path, directory, element_struct, latt_const, pot_element, dislocation_type, in_pot, pot_type,
