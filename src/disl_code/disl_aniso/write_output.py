...
"""
def output_directory(config, elem_property, dislocation_line_position, directory_path, ):
    '''
    ...
    line0 = str(dislocation_line_position[0]) + '_'
    line1 = str(dislocation_line_position[1]) + '/'
    directory = directory_path
    directory += '/' + element
    directory += '/' + case_name + cell_x_number + cell_y_number + cell_z_number
    directory += line0 + line1
...
def output_dir(main_path, pot_element, pot_name, element_struct, config_style, dislocation_type, 
    ...
    dir = os.path.join(main_path, pot_element, pot_name, element_struct, config_style, dislocation_type, simulation_type, )
                 # e.g. main/Ti/Ti_kevin_2020/hcp/cylinder/screw_a_basal/energy_minimization/
...
def write_in_file(pot_path, directory, element_struct, latt_const, pot_element, dislocation_type, in_pot, pot_type, 
...
def write_in_finite_T_meta_file(pot_path, directory, element_struct, latt_const, pot_element, dislocation_type, in_pot, pot_type,
                  mass, shell_radius, sample_center, disl_center, simulation_type, 
    ...
    file.write(f"dump           emin all custom {dump_interval} dump.{case_name}{calc}" 
                "dump_modify    emin element Ti sort id\n\n"
                "reset_timestep 0\n"
                "thermo         200\n"
                "thermo_style   custom step pe lx ly lz press\n"
